...
import soo.qna.service.QnaService;
...
public class QnaDAO implements QnaService {
	...
	public ArrayList<QnaDTO> qnaSelectAll() {
		...
		try {
			...
			String sql = "select num, qna_id, qna_title, qna_content, to_char(Qna_regisdate, 'yyyy-mm-dd') Qna_regisdate from qnaboard";
			log.info("SQL 확인-" + sql);
	...
	public QnaDTO qnaSelect(int num) {
		...
		try {
			...
			String sql = "select num, qna_id, qna_title, qna_content, qna_regisdate from qnaboard";
			sql += " where num=?";
			log.info("SQL 확인 -" +sql);
			preparedStatement = connection.prepareStatement(sql);
	...
	public boolean qnaInsert(QnaDTO qnaDTO) {
		 ...
		 try {
			...
			sql = "insert into qnaboard (num , qna_id, qna_title, qna_content, qna_regisdate)";
			sql += " values(?, ?, ?, ?, sysdate)";
			log.info("sql 확인-" + sql);
...
//			preparedStatement.setString(5, qnaDTO.getQna_regisdate());
	...
	public boolean qnaUpdate(QnaDTO qnaDTO) {
		...
		try {
			...
			DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc");
			connection = dataSource.getConnection( );
			String sql = "update qnaboard set qna_id=?, qna_title=?, qna_content=? ";
			sql += " where num=?";
			log.info("SQL 확인 - " + sql);
			preparedStatement = connection.prepareStatement(sql);
