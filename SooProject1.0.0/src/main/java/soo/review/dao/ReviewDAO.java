...
import soo.review.service.ReviewService;
...
public class ReviewDAO implements ReviewService {
	...
	public ArrayList<ReviewDTO> reviewSelectAll() {
		...
		try {
			...
			String sql = "select num, review_id, review_title, review_content, to_char(review_regisdate, 'yyyy-mm-dd') review_regisdate from reviewboard";
			log.info("SQL 확인-" + sql);
	...
	public ReviewDTO reviewSelect(int num) {
		...
		try {
			...
			String sql = "select num, review_id, review_title, review_content, review_regisdate from reviewboard";
			sql += " where num=?";
			log.info("SQL 확인 -" +sql);
			preparedStatement = connection.prepareStatement(sql);
	...
	public boolean reviewInsert(ReviewDTO reviewDTO) {
		int num = 0;
		 ...
		 try {
			...
			sql = "insert into reviewboard (num , review_id, review_title, review_content, review_regisdate)";
			sql += " values(?, ?, ?, ?, sysdate)";
			log.info("sql 확인-" + sql);
	...
	public boolean reviewUpdate(ReviewDTO reviewDTO) {
		...
		try {
			...
			DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc");
			connection = dataSource.getConnection( );
			String sql = "update reviewboard set review_id=?, review_title=?, review_content=? ";
			sql += " where num=?";
			log.info("SQL 확인 - " + sql);
			preparedStatement = connection.prepareStatement(sql);
